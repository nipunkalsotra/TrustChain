...
contract DeployTrustChain is Script {
    ...
    function run() external {
        ...
        identityRegistry.registerAgent(RESEARCHER_ID, researcherHash, RESEARCHER_MODEL, RESEARCHER_VERSION);
        console.log("   Registered: researcher");

        identityRegistry.registerAgent(VALIDATOR_ID, validatorHash, VALIDATOR_MODEL, VALIDATOR_VERSION);
        console.log("   Registered: validator");

        identityRegistry.registerAgent(SCORER_ID, scorerHash, SCORER_MODEL, SCORER_VERSION);
        console.log("   Registered: scorer");

        identityRegistry.registerAgent(REPORTER_ID, reporterHash, REPORTER_MODEL, REPORTER_VERSION);
        ...
        // ── PRINT SUMMARY ─────────────────────────────────────────────────
        _printSummary(deployer, address(trustScore), address(identityRegistry), address(auditLog));
    }
    ...
    function _envHashOrPlaceholder(string memory envKey, string memory agentId) internal view returns (bytes32) {
        ...
        console.log(
            string.concat(
                "   WARNING: ",
                envKey,
                " not set - using placeholder hash for ",
                agentId,
    ...
    function _printSummary(address deployer, address trustScore, address identityRegistry, address auditLog)
        internal
        view
    {
